...
class PDF extends FPDF
{
    function Header()
    {
        ...
        if($_GET['data'] == 0) {
            $this->Cell(0,10,utf8_decode('Orçamento do projeto'),0,5,'C');
        } else {
            $this->Cell(0,10,utf8_decode('Orçamento de ' . $_GET['data']),0,5,'C');
        }
    ...
    function BasicTable($header)
    {
        ...
        if($_GET['data'] == 0) {
            $mesOrcamento = $orcamento->selecionarDatasOrcamento($_SESSION['projeto']);
        } else {
            $mesOrcamento = $orcamento->selecionarDatasOrcamentoAno($_GET['data'], $_SESSION['projeto']);
        }

        while($mes = $mesOrcamento->fetchArray()) {

            $totalMes = 0;

            $nomedata = utf8_decode(strftime('%B de %Y', strtotime($mes['data_compra'])));
                                
            if(substr($nomedata, 0, 4) == 'mar?') {
                $nomedata = str_replace($nomedata, 'mar?', 'març') . substr($nomedata, 4, strlen($nomedata));
            }
            
            $this->Ln();
            $this->SetFont('Arial','b',10);
            $this->Cell(35,8,utf8_decode($nomedata),0, '', 'L');
            $this->Ln();
            ...
            $data = substr($mes['data_compra'], 0, 7);
            
            $orcamentoInfos = $orcamento->gerarInfosRelatorioMes($data, $_SESSION['projeto']);
            ...
            while($inf = $orcamentoInfos->fetchArray()) {
                ...
                $totalMes = $totalMes + $inf['valor_compra'];
            }
            $this->SetFont('Arial','b',10);
            $this->Ln();
            
            $this->Cell(190,8,utf8_decode('Total do mês: R$' . number_format($totalMes, 2, ',', '.')),0, '', 'R');
        }
